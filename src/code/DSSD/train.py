...
def train(cfg, args):
    ...
    optimizer = make_optimizer(cfg, model, lr) # uses SGD
    ...
    train_loader = make_data_loader(cfg, is_train=True, distributed=args.distributed, max_iter=max_iter, start_iter=arguments['iteration'], mode="train")
...
def main():
    # These add some default arguments to this script. We can either pass them from command line
    # or use these defaults.
    parser = argparse.ArgumentParser(description='DSSD: Deconvolutional Single Shot MultiBox Detector Training With PyTorch')
    parser.add_argument(
        "--config-file",
        default="",
        metavar="FILE",
    ...
    parser.add_argument('--eval_step', default=7000, type=int, help='Evaluate dataset every eval_step, disabled when eval_step < 0')
    ...
    #Check for GPU's and distribute accordingly
    ...
    #cfg has some default configurations loaded from dssd.config.defaults    
    ...
    if not args.skip_test:
        ...
        do_evaluation(cfg, model, distributed=args.distributed, mode='val')
