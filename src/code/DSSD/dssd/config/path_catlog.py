...
class DatasetCatalog:
    ...
    def get(name):
        if "voc" in name:
            ...
            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                data_dir=os.path.join( os.getcwd(), DatasetCatalog.DATA_DIR, attrs['data_dir']),
                split=attrs["split"],
            )
            print("Computed", args['data_dir'])
            return dict(factory="BDDDataset", args=args)
