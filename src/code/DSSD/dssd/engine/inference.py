...
from dssd.data.datasets.evaluation import evaluate

from dssd.utils import dist_util, mkdir
from dssd.utils.dist_util import synchronize, is_main_process
...
@torch.no_grad()
def do_evaluation(cfg, model, distributed, mode="test", **kwargs):
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        model = model.module
    model.eval()
    device = torch.device(cfg.MODEL.DEVICE)
    data_loaders_val = make_data_loader(cfg, is_train=False, distributed=distributed, mode=mode)
    eval_results = []
