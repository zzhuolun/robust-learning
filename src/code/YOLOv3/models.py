...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        if targets is None:
            ...
            # Return localization loss, classification loss, confidence loss
            return output, loss_x + loss_y + loss_w + loss_h, loss_cls, loss_conf
...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, regularized_layers=None, visualize=False):
        img_dim = x.shape[2]
        loss_loc, loss_cls, loss_conf = 0, 0, 0
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                x, layer_loss_loc, layer_loss_cls, layer_loss_conf = module[0](x, targets, img_dim)
                loss_loc += layer_loss_loc
                loss_cls += layer_loss_cls
                loss_conf += layer_loss_conf
                yolo_outputs.append(x)
            layer_outputs.append(x)

        if regularized_layers:
            activations = np.array(layer_outputs)[regularized_layers]
            return (loss_loc, loss_cls, loss_conf, activations)

        yolo_outputs = to_cpu(torch.cat(yolo_outputs, 1))
        return yolo_outputs if targets is None else (loss_loc, loss_cls, loss_conf, yolo_outputs)
